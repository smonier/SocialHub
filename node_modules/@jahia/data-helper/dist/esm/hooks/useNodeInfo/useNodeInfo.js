...
var timeoutHandler = function timeoutHandler(client) {
  ...
  mergedQueue.forEach(function (mergedRequest) {
    ...
    var skip;
    try {
      var r = getQuery(variables, options);
      query = r.query;
      generatedVariables = r.generatedVariables;
      skip = r.skip;
    } catch (e) {
      var error = {
        message: "One of the batched queries resulted in error: ".concat(e.message)
      };
    ...
    if (skip) {
      ...
      var observableQuery = client.watchQuery(_objectSpread(_objectSpread({
        query: query,
        errorPolicy: 'ignore'
      }, queryOptions), {}, {
        variables: generatedVariables
      }));
      var subscription = observableQuery.subscribe(function (_ref) {
        var data = _ref.data,
  ...
  if (node && options) {
    ...
    if (options.getSubNodesCount) {
      options.getSubNodesCount.forEach(function (name) {
        ...
        node['subNodesCount_' + name] = res === null || res === void 0 || (_res$pageInfo = res.pageInfo) === null || _res$pageInfo === void 0 ? void 0 : _res$pageInfo.totalCount;
      });
    }
  }
  return node;
